...
⚠️ 反向操作提示：在 C++ 中主动修改 QML 控件（不推荐）  
虽然可以通过 engine.rootObjects().first()->findChild<QObject*>() 在 C++ 里主动去抓取和修改 QML 界面上的某个特定按钮或输入框，但在实际开发中极其不推荐这样做。

* 原因：这会导致 C++ 代码与 UI 布局严重耦合。一旦前端 QML 把界面结构改了，C++ 就会因为找不到控件而直接崩溃。
* 黄金法则：UI 永远主动找数据，数据永远被动通知 UI。即通过上述的“方向一（属性绑定）”和“方向二（上下文对象）”来驱动界面。
